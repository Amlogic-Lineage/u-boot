...
/*
 * The legacy i2c_read()/i2c_write() API is gone in this u-boot; the TCA6408
 * shares the AO i2c bus with the KBI MCU, so reuse the DM-based helpers
 * exported by cmd/amlogic/cmd_kbi.c.
 */
...
static int tca6408_write_reg(u8 reg, u8 val)
{
	...
	error = khadas_i2c_write(chip.addr, reg, val);
	if (error < 0) {
...
static int tca6408_read_reg(u8 reg, u8 *val)
{
	...
	retval = khadas_i2c_read(chip.addr, reg, val);
	if (retval < 0) {
