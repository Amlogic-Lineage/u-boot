...
#define AML_VDDEE_INIT_VOLTAGE		880		// VDDEE power up voltage
...
/* CONFIG_MDUMP_COMPRESS pulls in common/ramdump.c, which needs REG_MDUMP_*
 * registers that g12b does not define. Disabled, as on g12b_w400_v1. */
#if 0
...
/*
 * CONFIG_BOOTLOADER_CONTROL_BLOCK, CONFIG_CMD_BOOTCTOL_AVB and
 * CONFIG_AML_FACTORY_PROVISION are Kconfig symbols in this u-boot;
 * they are selected from board/khadas/defconfigs/kvim3l_defconfig.
 */
...
#endif
#define AML_IR_REMOTE_POWER_UP_KEY_VAL1 0xeb14ff00 //khadas ir --- power
...
/* ddr functions: these are Kconfig symbols now, see kvim3_defconfig */
#if 0
...
#endif
/* bare names consumed by asm/arch/ddr_define.h when building acs/timing.c */
...
/* MMC is driver-model based now (CONFIG_DM_MMC + CONFIG_MMC_MESON_GX) */
...
/*
 * meson SPI / SPI flash: all of these are Kconfig symbols in this u-boot and
 * are selected from board/khadas/defconfigs/kvim3l_defconfig.
 */
#if 0
...
/* DISPLAY & HDMITX */
/* CONFIG_AML_HDMITX / CONFIG_AML_HDMITX20 come from Kconfig now */
...
/* PWM DM driver: CONFIG_DM_PWM/CONFIG_PWM_MESON come from Kconfig now */
...
/* needs REG_MDUMP_* registers that g12a does not define */
#if 0
#define CONFIG_MDUMP_COMPRESS 1
#endif
